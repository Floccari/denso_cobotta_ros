...
public:
  ...
  bool change(const uint32_t color);
  bool change(const uint8_t blink, const uint8_t red, const uint8_t green, const uint8_t blue);
  bool change(const struct LedColor color);
  bool change(const enum LedColorTable color);

  void forceChange(const uint32_t color);
  void forceChange(const uint8_t blink, const uint8_t red, const uint8_t green, const uint8_t blue);
  void forceChange(const struct LedColor color);
  void forceChange(const enum LedColorTable color);

  uint32_t receiveColor(void);
  static struct LedColor toLedColor(const uint32_t state);
  static uint32_t toUint32(const struct LedColor color);

private:
  static void writeHw(int fd, uint32_t color);
  static uint32_t readHw(int fd);
