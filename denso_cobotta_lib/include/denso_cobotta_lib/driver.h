...
public:
  ...
  void clearError();
  StateCode dequeue(long arm_no);
    DriverStateInfo receiveAllState();
  uint32_t getStateQueueSize(long arm_no) const;
  DriverVersion getVersion() const;
  bool isError() const;
  bool isFatalError() const;
  bool isIpResetButtonOn() const;

  static void checkArmNo(long arm_no);
  static void checkJointNo(long joint_no);

  static struct DriverCommandInfo writeHwUpdate(int fd, const SRV_COMM_SEND& send_data);
  static SRV_COMM_RECV readHwEncoder(int fd, long arm_no);
...
private:
  static DriverVersion readHwVersion(int fd);
  static void writeHwClear(int fd);
  static StateCode readHwQueue(int fd, long arm_no);
  static DriverStateInfo readHwState(int fd);
